...
use halo2_exactness_tooling::baseline_a::{
    ADVICE_COLS, FIXED_COLS, INSTANCE_COLS, LIN_CONSTRAINTS_PER_REP, LOOKUP_CELLS_PER_REP,
    MUL_CONSTRAINTS_PER_REP, ROWS_PER_REP, BaselineASecureCircuit,
};
...
fn find_k_min(circuit: &BaselineASecureCircuit<Fr>) -> Result<u32, String> {
    ...
    Err(format!(
        "failed to find feasible k in range [{MIN_K}, {MAX_K}] for A_secure"
    ))
}
...
fn run() -> Result<(), String> {
    ...
    let circuit = BaselineASecureCircuit::<Fr>::with_profile(scale, args.input_profile);
    ...
    let k_min = if let Some(v) = args.known_k_min {
        if !(MIN_K..=MAX_K).contains(&v) {
            return Err(format!(
                "provided --known-k-min={v} is outside [{MIN_K}, {MAX_K}] for A_secure"
            ));
        }
        v
    } else {
    ...
    let k_run = if let Some(v) = args.k_run_override {
        if v < k_min {
            return Err(format!(
                "requested --k-run={v} is below k_min={k_min} for A_secure at scale={scale}"
            ));
        }
        if v > MAX_K {
            return Err(format!(
                "requested --k-run={v} exceeds MAX_K={MAX_K} for A_secure binary"
            ));
        }
        v
    } else {
...
\"status\":\"full-local-ok\",\"notes\":\"A_secure full-local measured with real keygen/prove/verify (Blake2b transcript); input_profile={input_profile}\"}}"
    );
    println!("{payload}");
    Ok(())
}
