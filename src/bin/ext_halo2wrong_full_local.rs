...
use rand_core::OsRng;
use halo2_exactness_tooling::external_h2w::{
    ExternalH2W66BitCircuit, ADVICE_COLS, FIXED_COLS, INSTANCE_COLS, LIN_CONSTRAINTS_PER_REP,
    LOOKUP_CELLS_PER_REP, MUL_CONSTRAINTS_PER_REP,
};
...
fn find_k_min(circuit: &ExternalH2W66BitCircuit<Fr>) -> Result<u32, String> {
    for k in MIN_K..=MAX_K {
        let maybe_prover = catch_unwind_silent(|| MockProver::run(k, circuit, vec![vec![]]));
        if let Ok(Ok(_)) = maybe_prover {
            return Ok(k);
        }
    }
    Err(format!(
        "failed to find feasible k in range [{MIN_K}, {MAX_K}] for ext_halo2wrong"
    ))
}
...
fn run() -> Result<(), String> {
    ...
    let k_run = if let Some(v) = args.k_run_override {
        ...
        if v > MAX_K {
            return Err(format!(
                "requested --k-run={v} exceeds MAX_K={MAX_K} for ext_halo2wrong binary"
            ));
        }
        v
    } else {
...
\"status\":\"full-local-ok\",\"notes\":\"ext_halo2wrong full-local measured with row-family-equivalent maingate::MainGate::to_bits decomposition workload\"}}"
    );
    println!("{payload}");
    Ok(())
}
